import noProjectImage from '../assets/no-projects.png'
import Button from './Button'

export default function NoProjectSelected({onStartNewProject}) {
    return(
        <div className=' mt-24 text-center w-2/3'>
            <img src={noProjectImage} alt='empty task' className='w-16 h-16 object-contain mx-auto'></img>
            <h2 className='text-xl font-bold text-stone-500 my-4'>No Project Selected</h2>
            <p className=' text-stone-400 mb-4'>Select a project or sterted with a new one</p>
            <p className=' mt-8'>
                {/* <button className=''>Create new project</button> */}
                <Button onClick={onStartNewProject} children={'Create new project'}/>
            </p>
        </div>
    )
}